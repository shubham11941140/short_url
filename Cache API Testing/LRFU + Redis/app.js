...
const WeakLRUCache = require('weak-lru-cache').WeakLRUCache

const myCache = new WeakLRUCache((cacheSize = 80000))
...
app.post('/api/readshorten/:apiKey', async function (req, res) {
  ...
  const cahcedVal = myCache.getValue(link)
  if (cahcedVal != undefined) {
    res.json({ longURL: cahcedVal })
  } else {
